...
def cfg_test():
    cfg = get_cfg()
    cfg.merge_from_file(model_zoo.get_config_file(MODEL_PATH))
    cfg.MODEL.WEIGHTS = os.path.join(cfg.OUTPUT_DIR, 'model_final_res.pth')
...
from detectron2.utils.visualizer import ColorMode
# dataset_dicts = get_wire_dicts("dataset_wire/val")
txt_path = 'mask_zolyator.txt'
for i, d in enumerate(glob.glob('./img_test_izo/*.jpg')):
    im = cv2.imread(d)
    ...
    cv2.imwrite(os.path.join('out_img_izo', '{}.jpg'.format(i)),out.get_image()[:, :, ::-1])
    # cv2.waitKey()